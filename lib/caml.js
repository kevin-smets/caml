...
/**
 * Replaces variables in the yaml string. A variable is in the form ${variable}
 * @param yamlString
 * @param variables
 */
function replaceVariables(yamlString, variables) {
  var template = '${%s}';
  Object.keys(variables).forEach(function (key) {
    var value = variables[key];
    var variable = util.format(template, key);
    yamlString = yamlString.replace(new RegExp(escape(variable), 'g'), value);
  });
  return yamlString;
}
...
function camlize(options) {
  ...
  if (options.variables) {
    yamlString = replaceVariables(yamlString, options.variables);
  }
