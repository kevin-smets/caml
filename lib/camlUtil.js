 ...
 * @returns {String}
 */
module.exports.parseIncludes = function (yaml, dir) {
  const fs = require('fs');
  const os = require('os');
  ...
  while (hasInclude) {
    ...
    for (var i = 0; i < yamlLines.length; i++) {
      ...
      if (line.trim().length > 0 && line.trim().indexOf("#") > -1 && line.indexOf("@include") > -1) {
        //   hasInclude = true;

        var include = line.replace(/#.*@include\s*/, '');

        // Add .yml if need be
        // istanbul ignore else
        if (path.extname(include) !== 'yml') {
          include = include + ".yml";
        }
        ...
        var includeContentLines = "";

        try {
          includeContentLines = fs.readFileSync(include, "UTF8").split(/\r?\n/);
        } catch (err) {
          console.error("File to include " + include + ' not found.');
          throw (err);
        }

        yamlLines.splice.apply(yamlLines, [index, 1].concat(includeContentLines));
